...
class Camera:
    ...
    def distort_points(self, points):
        ...
        out, _ = cv2.projectPoints(
            new_points, np.zeros(3), np.zeros(3), self.matrix.astype("float64"), self.dist.astype("float64")
        )
    ...
    def project(self, points):
        ...
        out, _ = cv2.projectPoints(
            points, self.rvec, self.tvec, self.matrix.astype("float64"), self.dist.astype("float64")
        )
        return out
...
class FisheyeCamera(Camera):
    ...
    def undistort_points(self, points):
        ...
        out = cv2.fisheye.undistortPoints(
            points.astype("float64"), self.matrix.astype("float64"), self.dist.astype("float64")
        )
        return out.reshape(shape)
...
class CameraGroup:
    ...
    def triangulate_possible(self, points, undistort=True, min_cams=2, progress=False, threshold=0.5):
        ...
        for point_ix in iterator:
            ...
            for picked in itertools.product(*all_iters[point_ix].values()):
                ...
                if err < best_error:
                    best_point = {"error": err, "point": p3d[:3], "points": pts, "picked": picked, "joint_ix": point_ix}
                    best_error = err
    ...
    def reprojection_error(self, p3ds, p2ds, mean=False):
        ...
        assert p3ds.shape == (n_points, 3), "shapes of 2D and 3D points are not consistent: " "2D={}, 3D={}".format(
    ...
    def bundle_adjust_iter(
        ...
        for i in range(n_iters):
            ...
            self.bundle_adjust(p2ds_samp, extra_samp, loss="linear", ftol=ftol, max_nfev=max_nfev, verbose=verbose)
        ...
        extra_good = subset_extra(extra, good)
        self.bundle_adjust(
            p2ds[:, good], extra_good, loss="linear", ftol=ftol, max_nfev=max(200, max_nfev), verbose=verbose
        )
    ...
    def optim_points_possible(
        ...
        jac = self._jac_sparsity_triangulation_possible(
            points, constraints=constraints, constraints_weak=constraints_weak, n_deriv_smooth=n_deriv_smooth
        )
    ...
    def calibrate_rows(self, all_rows, board, init_intrinsics=True, init_extrinsics=True, verbose=True, **kwargs):
    ...
    def calibrate_videos(self, videos, board, init_intrinsics=True, init_extrinsics=True, verbose=True, **kwargs):
        ...
        error = self.calibrate_rows(
            all_rows, board, init_intrinsics=init_intrinsics, init_extrinsics=init_extrinsics, verbose=verbose, **kwargs
        )
